...
export class BoundingBox {
     ...
     */
    get changed() {
        const ret = wasm.__wbg_get_boundingbox_changed(this.__wbg_ptr);
        return ret !== 0;
    }
    /**
     * @param {boolean} arg0
     */
    set changed(arg0) {
        wasm.__wbg_set_boundingbox_changed(this.__wbg_ptr, arg0);
    }
}
 ...
 * @param {Uint8Array} prev_frame
 * @param {number} width
 * @param {number} height
 * @param {number} threshold
 * @returns {BoundingBox}
 */
export function calculate_diff(current_frame, prev_frame, width, height, threshold) {
    const ptr0 = passArray8ToWasm0(current_frame, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArray8ToWasm0(prev_frame, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.calculate_diff(ptr0, len0, ptr1, len1, width, height, threshold);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('fovea_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
